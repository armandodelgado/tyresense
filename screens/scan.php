...
<!-- Camera zone -->
<form action="?screen=process_scan" method="POST" enctype="multipart/form-data" id="scanForm">
  <label class="scan-zone" style="cursor:pointer; display:flex; flex-direction:column; align-items:center;">
    <?= svgCamera() ?>
    <span class="scan-zone-label">Toca para tomar foto</span>
    <span class="scan-zone-sub">Mantén la cámara a 30&ndash;50 cm</span>
    <input type="file" name="tire_image" accept="image/*" capture="environment" style="display:none" onchange="document.getElementById('scanForm').submit();">
  </label>
</form>
